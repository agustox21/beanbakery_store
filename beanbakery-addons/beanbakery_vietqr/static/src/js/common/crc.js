...
/**
 * This function use to make VietQR - a Napas QR Standard
 * @param {*} bank_holder : Bank holder name
 * @param {*} bank_account : Bank account number
 * @param {*} bank_code : Bank code including
 * @param {*} bill_value : Bill value
 * @param {*} bill_no : Bill number
 * @param {*} bill_detail : Bill detail, max 50 chars
 * @param {*} account_type : default is true , True is using bank account, False is using ATM Card Number
 * @param {*} dynamic_qrcode : default is true, True is using dynamic QRcode type, False is using static QRcode type
 * @returns QRcode <img> tag
 * List of bank code
...
function dynamicQrcode( bank_holder = "NGUYEN PHAM THUY LAN", bank_account = "129005718", bank_code = "VPBank", bill_value="10000",bill_no="", bill_detail="Thanh toan don hang cho Bean Bakery",account_type = true, dynamic_qrcode = true ){
  ...
  let txn_purpose = "08" + (bill_detail.length<10? "0"+bill_detail.length.toString() : bill_detail.length.toString()) + nonAccentVietnamese(bill_detail);
  let bill_number = bill_no.length >0 ? "01" + (bill_no.length < 10 ? "0"+ bill_no.length.toString() : bill_no.length.toString()) + bill_no.replace(new RegExp('/', 'g'),' ') : ""
